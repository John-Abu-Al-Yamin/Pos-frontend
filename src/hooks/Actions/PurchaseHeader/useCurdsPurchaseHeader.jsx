...
export const useGetAllPurchaseHeaders = (page = 1, per_page = 12, filters = {}) => {
  const params = {
    page,
    per_page,
    ...Object.fromEntries(
  ...
  const { data, isPending, refetch, ...rest } = useGetData({
    ...
    params,
    queryKeys: [queryKeys.purchaseHeaders, page, per_page, JSON.stringify(filters)],
  });
...
export const useCompletePurchaseHeaders = () => {
  const {
  ...
  const mutate = (id, options) => {
    originalMutate(
      { url: `${endPoints.purchaseHeaders}/${id}/complete` },
      options,
    );
  };
...
export const useCancelPurchaseHeaders = () => {
  const {
    mutate: originalMutate,
    data,
    error,
  ...
  const mutate = (id, options) => {
    originalMutate(
      { url: `${endPoints.purchaseHeaders}/${id}/cancel` },
      options,
    );
  };
