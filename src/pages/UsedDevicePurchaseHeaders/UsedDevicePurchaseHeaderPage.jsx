...
import {
  useGetAllUsedPurchaseHeaders,
  useCompleteUsedPurchaseHeaders,
  useCancelUsedPurchaseHeaders,
...
const UsedDevicePurchaseHeaderPage = () => {
  ...
  const { data, isPending } = useGetAllUsedPurchaseHeaders(page, per_page, filters);
  const { data: customersData } = useGetAllCustomers(1, 100);
  const { mutate: completeMutate } = useCompleteUsedPurchaseHeaders();
  const { mutate: cancelMutate } = useCancelUsedPurchaseHeaders();
  ...
  return (
    <div>
      <CustomHeader
        title="فواتير شراء الأجهزة المستعملة"
        description="قائمة فواتير شراء الأجهزة المستعملة"
        buttonText="فاتورة شراء أجهزة مستعملة"
        onButtonClick={() => navigate("/used-purchase-headers/add")}
      />
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          navigate(`/used-purchase-headers/details/${header.id}`)
                        }
                      >
                      ...
                      {header.status === "draft" && (
                        <>
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              navigate(`/used-purchase-headers/update/${header.id}`)
                            }
                          >
                          ...
                          <Button
                            variant="default"
                            size="sm"
                            disabled={header.used_device_purchase_items_count <= 0}
                            title={header.used_device_purchase_items_count <= 0 ? "يجب إضافة أصناف للفاتورة أولاً" : ""}
...
export default UsedDevicePurchaseHeaderPage;
