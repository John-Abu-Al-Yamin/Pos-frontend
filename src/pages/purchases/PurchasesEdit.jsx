...
} from "@/hooks/Actions/PurchaseHeaders/useCurdsPurchaseHeaders";
...
const PurchasesEdit = () => {
  ...
  // Fetch suppliers list (high limit to get all)
  const { data: suppliersData, isPending: suppliersPending } =
    useGetAllSuppliers(1, 1000);
  const suppliers = suppliersData?.data?.data ?? [];

  // Fetch current purchase data
  const { data: purchaseDetails, isPending: purchasePending } =
    useGetPurchaseHeadersById(id);
  const purchase = purchaseDetails?.data?.data ?? purchaseDetails?.data;

  // Update purchase mutation
  const { mutate: updatePurchase, isPending: submitPending } =
  ...
  React.useEffect(() => {
    if (purchase) {
      ...
      form.reset({
        ...
        date: purchase.date ? new Date(purchase.date) : null,
        reference: purchase.reference ?? "",
        type: purchase.type ?? "purchase",
      });

      // Only show the form AFTER reset — ensures Select mounts with correct value
      setInitialized(true);
    }
  ...
  if (purchasePending || suppliersPending || !initialized) {
    return <Loading />;
  }
  ...
  return (
    <div className=" mx-auto p-4">
      <AddEditHeader
        ...
        description="قم بتعديل تفاصيل فاتورة المشتريات"
        backPath="/purchases"
