...
const MaintenancePartsTable = ({ data, isPending }) => {
  if (isPending) {
    return (
      <Card className="mb-6">
        <CardHeader>
  ...
  return (
    <Card className="mb-6">
      <CardHeader>
        <CardTitle className="text-base">قطع الغيار المستخدمة</CardTitle>
      </CardHeader>
            ...
            <TableBody>
              {data.map((item) => (
                <TableRow key={item.product_id}>
                  ...
                  <TableCell>
                    {formatCurrency((item.total_revenue || 0) - (item.total_cost || 0))}
                  </TableCell>
                </TableRow>
              ))}
              {data.length === 0 && (
...
export default MaintenancePartsTable;
